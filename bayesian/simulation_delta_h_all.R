...
true_params <- base_scatter +
    ...
    labs(
        title = "Simulated Hardiness and Temperature Data with True Linear Parameters",
...
posterior_preds <- data %>%
    ...
    hardiness = seq_range(hardiness, n = 101)
    ) %>%
    add_epred_draws(fit, ndraws = 100) %>%
    ggplot(aes(x = mean_temp, y = hardiness, color = phase)) +
