...
sb_eco <- sb_oe[sb_oe$DD_sum < -300, ]

ylim_min <- min(na.omit(sb_eco$hardiness_delta)) - 0.5
ylim_max <- max(na.omit(sb_eco$hardiness_delta)) + 0.5
...
for (season in unique(hardiness$season)) {
    ...
    data = sb_eco[sb_eco$season == season, ],
...
mtext("Seasonal Plots of Hardiness Change in Ecodormancy",
