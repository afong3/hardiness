...
setwd("c:/users/adamf/onedrive/documents/code/hardiness/final")
...
rawlong.tot2 <- read.csv("rawlong.tot2.csv")
rawlong.nodups <- read.csv("rawlong.nodups.csv")
...
syncmodel <- stan("twolevelrandomslope.stan", data=c("N","Nspp","y","species","year"),
                   iter=4000, warmup=3000, chains=4, cores=4)
save(syncmodel, file="output/syncmodel.Rdata")

# See the stan code on this model for notes on what it does
syncmodel2 <- stan("twolevelrandomslopeint.stan", data=c("N","Nspp","y","species","year"),
                   iter=4000, warmup=3000, chains=4, cores=4)
...
plot(range(year), range(y), type="n", xlab="Year",
     ylab="Day of year", bty="l", main="Raw real data")
for (j in 1:Nspp){
  lines(year[species==j], y[species==j])
}
hist(y, xlab="Day of year", main="Real data")
dev.off()
...
# Create the data using new a and b for each of the species, 100 times
...
hist(colMeans(y.sd100), col=rgb(1,0,0,0.5), xlim=c(10,14), ylim=c(0,35),
    main="Mean(SD of y) from 100 simulated datasets based on Stan model",
    xlab="mean(SD of one sim)")
